const moods = {
  "afraid": {
    verse: "Isaiah 41:10",
    advice: "God holds you in His hands—fear not.",
    songs: ["Fear Is a Liar - Zach Williams", "Whom Shall I Fear - Chris Tomlin"]
  },
  "angry": {
    verse: "James 1:19",
    advice: "Pause before you act. Respond with peace.",
    songs: ["Control - Tenth Avenue North", "Jesus, Friend of Sinners - Casting Crowns"]
  },
  "anxious": {
    verse: "Philippians 4:6",
    advice: "Take a deep breath and give your worries to God.",
    songs: ["Peace Be Still - Hope Darst", "Oceans - Hillsong UNITED", "Same God - Elevation Worship"]
  },
  "bored": {
    verse: "Ecclesiastes 3:1",
    advice: "There’s a purpose in the stillness too.",
    songs: ["Seasons - Hillsong Worship", "While I'm Waiting - John Waller"]
  },
  "confused": {
    verse: "Proverbs 3:5-6",
    advice: "Trust His plan, even when it’s unclear.",
    songs: ["Way Maker - Sinach", "You Say - Lauren Daigle"]
  },
  "content": {
    verse: "Philippians 4:11-12",
    advice: "Find joy and peace in your current circumstances.",
    songs: ["Goodness of God - Bethel Music", "It Is Well With My Soul - Hymn", "Million Little Miracles - Elevation Worship & Maverick City"]
  },
  "discouraged": {
    verse: "Isaiah 40:31",
    advice: "Wait on the Lord; He will renew your strength.",
    songs: ["Praise You In This Storm - Casting Crowns", "Greater - MercyMe"]
  },
  "doubtful": {
    verse: "James 1:6",
    advice: "Ask God for wisdom with full faith.",
    songs: ["Trust in You - Lauren Daigle", "Strong God - Meredith Andrews"]
  },
  "forgiven": {
    verse: "Psalm 103:12",
    advice: "Your sins are cast as far as the east from the west.",
    songs: ["Redeemed - Big Daddy Weave", "Grace Wins - Matthew West"]
  },
  "frustrated": {
    verse: "Psalm 37:7",
    advice: "Breathe. Patience brings peace.",
    songs: ["Firm Foundation - Cody Carnes & The Belonging Co.", "Still - Hillsong"]
  },
  "grateful": {
    verse: "Colossians 3:15",
    advice: "Let gratitude fill your thoughts today.",
    songs: ["Gratitude - Brandon Lake", "Thank You Jesus - Hillsong", "Praise - Elevation Worship"]
  },
  "guilty": {
    verse: "1 John 1:9",
    advice: "Confess your sins to God, and He will cleanse you of your guilt.",
    songs: ["Clean - Natalie Grant", "Forgiven - Crowder"]
  },
  "hopeful": {
    verse: "Romans 15:13",
    advice: "Hope is powerful—let it shine today.",
    songs: ["Hope in Front of Me - Danny Gokey", "Living Hope - Phil Wickham"]
  },
  "inspired": {
    verse: "Ephesians 2:10",
    advice: "You are His masterpiece, created for good works.",
    songs: ["Do It Again - Elevation Worship", "Limitless - Colton Dixon"]
  },
  "joyful": {
    verse: "Psalm 100:1",
    advice: "Let your joy be a testimony of God’s goodness.",
    songs: ["Joy - For King & Country", "Happy Dance - MercyMe", "Praise - Elevation Worship"]
  },
  "lonely": {
    verse: "Deuteronomy 31:6",
    advice: "You are never truly alone. God walks with you.",
    songs: ["Never Alone - Hillsong Young & Free", "Never - Tasha Layton"]
  },
   "loved": {
    verse: "Romans 5:8",
    advice: "Embrace it, and let it flow out to others, showing compassion and kindness, just as God shows it to you. ",
    songs: ["Love Of God - Phil Wickman", "Reckless Love - Bethel Music", "Unending Love - Jadwin Gillies"]
  },
  "motivated": {
    verse: "Philippians 4:13",
    advice: "You’ve got this, with strength from above.",
    songs: ["Rise - Danny Gokey", "Warrior - Hannah Kerr", "Gold - Britt Nicole"]
  },
  "overwhelmed": {
    verse: "Psalm 61:2",
    advice: "When your heart is faint, lead it to the Rock.",
    songs: ["Overwhelmed - Big Daddy Weave", "Even If - MercyMe", "In Jesus Name - Katy Nichole", "Be Alright - Evan Craft, Danny Gokey, Redimi2"]
  },
  "peaceful": {
    verse: "John 14:27",
    advice: "Keep walking in that calm. It’s beautiful.",
    songs: ["It Is Well - Kristene DiMarco", "Be Still - Hillsong Worship"]
  },
  "protective": {
    verse: "Psalm 91:4",
    advice: "He will cover you with His feathers, and under His wings you will find refuge.",
    songs: ["The Blessing - Kari Jobe, Cody Carnes, Elevation Worship", "Defender - Francesca Battistelli"]
  },
  "reflective": {
    verse: "Psalm 139:23-24",
    advice: "Search your heart and let God lead you.",
    songs: ["So Will I (100 Billion X) - Hillsong UNITED", "My Lighthouse - Rend Collective"]
  },
  "sad": {
    verse: "Psalm 34:18",
    advice: "He is close to the brokenhearted. Lean on Him.",
    songs: ["Thy Will - Hillary Scott", "Even When It Hurts - Hillsong UNITED", "It Is Well (Through It All) - Bethel Music", "Work It Out - ABLAZE"]
  },
  "strong": {
    verse: "Philippians 4:13",
    advice: "You can do all things through Christ who strengthens you.",
    songs: ["Raise a Hallelujah - Bethel Music", "Unstoppable God - Sanctus Real"]
  },
  "thankful": {
    verse: "1 Thessalonians 5:18",
    advice: "Keep a spirit of gratitude. Everything is a gift.",
    songs: ["Thank You Lord - Chris Tomlin", "Blessings - Laura Story", "I Thank God - Housefires"]
  },
  "tired": {
    verse: "Matthew 11:28",
    advice: "Rest in Him, you’ve done enough today.",
    songs: ["Come to Me - Bethel Music", "Worn - Tenth Avenue North"]
  },
  "vulnerable": {
    verse: "2 Corinthians 12:9",
    advice: "His grace is sufficient for you, in your weakness.",
    songs: ["Scars in Heaven - Casting Crowns", "Rescue - Lauren Daigle"]
  }
};
